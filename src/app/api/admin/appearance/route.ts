...
import { isInvitationFontId } from "@/config/invitation-fonts";
...
export async function PUT(request: Request) {
  ...
  if (!parsed.success || !isInvitationThemeId(parsed.data.themeId) || !isInvitationFontId(parsed.data.fontId)) {
    return noStoreJson({ message: "Giao diện thiệp không hợp lệ." }, { status: 400 });
  }
