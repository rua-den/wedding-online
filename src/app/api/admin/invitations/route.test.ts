...
describe("/api/admin/invitations", () => {
  it("rejects unauthenticated writes before parsing the body", async () => {
    const createResponse = await POST(request("/api/admin/invitations", { method: "POST", body: "not-json" }, false));
    const deleteResponse = await DELETE(request("/api/admin/invitations", { method: "DELETE", body: "not-json" }, false));
  ...
  it("deletes an invitation and returns the refreshed summary", async () => {
    ...
    const deleted = await DELETE(request("/api/admin/invitations", {
      method: "DELETE", body: JSON.stringify({ code: createdBody.invitation.code }),
    }));

    expect(deleted.status).toBe(200);
    await expect(deleted.json()).resolves.toMatchObject({
      deleted: { code: createdBody.invitation.code, name: "Khách test", hadRsvp: false },
      summary: { invitationCount: 0 },
    });
  });

  it("returns 404 when deleting an unknown invitation", async () => {
    const response = await DELETE(request("/api/admin/invitations", {
      method: "DELETE", body: JSON.stringify({ code: "missing-code" }),
    }));
  ...
  it("returns 400 for malformed JSON", async () => {
    ...
    expect((await DELETE(request("/api/admin/invitations", { method: "DELETE", body: "{" }))).status).toBe(400);
  });
});
