...
const invitations: AdminInvitation[] = [
  {
    ...
    guestCount: 2,
    rsvpUpdatedAt: "2026-01-02T00:00:00.000Z",
  },
  {
    code: "minh-xyz",
    name: "Anh Minh",
...
describe("AdminDashboard", () => {
  ...
  it("creates an invitation then renders its copyable link", async () => {
    ...
    const fetcher = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          invitation: { ...invitations[0], name: "Cô Lan mới", code: "co-lan-moi" },
          invitationUrl: "http://localhost:3000/moi/co-lan-moi",
          summary,
        }),
        { status: 201 },
      ),
  ...
  it("toggles an invitation active state through the protected API", async () => {
    const user = userEvent.setup();
    const fetcher = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ invitation: { ...invitations[0], active: false }, summary }), { status: 200 }),
    );
  ...
  it("edits an invitation name and guest limit", async () => {
    const user = userEvent.setup();
    const fetcher = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ invitation: { ...invitations[0], name: "Cô Lan thân mến", maxGuests: 3 }, summary }), { status: 200 }),
    );
  ...
  it("announces mutation errors and exposes mobile table scrolling", async () => {
    const user = userEvent.setup();
    const fetcher = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ message: "Không thể tạo thiệp mời." }), { status: 500 }),
    );
