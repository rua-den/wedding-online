...
export function Invitation({
  ...
  return <main>
    ...
    <section className="countdown-section section-shell" id="ngay-chung-doi" aria-labelledby="countdown-title">
      <p className="eyebrow">{copy.countdown.eyebrow}</p>
      <h2 id="countdown-title">{copy.countdown.title}</h2>
      <VinylRecord groom={couple.shortGroomName} bride={couple.shortBrideName} />
      <p className="section-copy">{event.dateLabel} · {event.timeLabel}</p>
        ...
        {copy.story.milestones.map((milestone, index) => {
          ...
          return <li className={`timeline-item timeline-item-${milestone.imagePosition}`} key={`${milestone.title}-${index}`}>
            ...
            <div className="timeline-entry">
              ...
              <div className="timeline-copy"><p className="timeline-date">{milestone.date}</p><h3>{milestone.title}</h3><p>{milestone.description}</p></div>
            </div>
          </li>;
        })}
      </ol>
