  ...
  return <main>
    <section className={`hero section-shell${hero ? " has-hero-media" : ""}`} aria-labelledby="invitation-title">
      ...
      <div className="hero-frame">
        ...
        <a className="scroll-cue" href="#ngay-chung-doi">{cover.scrollCue} <span aria-hidden="true">↓</span></a>
      </div>
    </section>

    <section className="countdown-section section-shell" id="ngay-chung-doi" aria-labelledby="countdown-title">
      <p className="eyebrow">{copy.countdown.eyebrow}</p><h2 id="countdown-title">{copy.countdown.title}</h2><p className="section-copy">{event.dateLabel} · {event.timeLabel}</p><Countdown eventTime={event.dateTime} />
    </section>

    <section className="couple-section section-shell" aria-labelledby="couple-title"><div className="section-heading"><p className="eyebrow">{copy.coupleSection.eyebrow}</p><h2 id="couple-title">{copy.coupleSection.title}</h2></div>
      <div className="couple-grid">
        <article className="person-card">{groom ? <MediaFrame asset={groom} className="portrait portrait-groom media-frame-slot-portrait" alt={groom.alt || couple.groom} loading="lazy" /> : <div className="portrait portrait-groom"><span aria-hidden="true">{couple.shortGroomName.charAt(0)}</span></div>}<p className="person-role">{copy.coupleSection.groomRole}</p><h3>{couple.groom}</h3><p>{couple.groomBio}</p></article>
        <div className="heart-connector" aria-hidden="true">♡</div>
        <article className="person-card">{bride ? <MediaFrame asset={bride} className="portrait portrait-bride media-frame-slot-portrait" alt={bride.alt || couple.bride} loading="lazy" /> : <div className="portrait portrait-bride"><span aria-hidden="true">{couple.shortBrideName.charAt(0)}</span></div>}<p className="person-role">{copy.coupleSection.brideRole}</p><h3>{couple.bride}</h3><p>{couple.brideBio}</p></article>
      </div>
    </section>

    <section className="story-section section-shell" aria-labelledby="story-title"><div className="section-heading"><p className="eyebrow">{copy.story.eyebrow}</p><h2 id="story-title">{copy.story.title}</h2></div>
      <div className={`story-layout${storyImage ? " has-image" : ""}`}>{storyImage ? <MediaFrame asset={storyImage} className="story-image media-frame-slot-story" alt={storyImage.alt || `Khoảnh khắc của ${couple.shortGroomName} và ${couple.shortBrideName}`} loading="lazy" /> : null}<ol className="timeline">{copy.story.milestones.map((milestone, index) => <li key={`${milestone.title}-${index}`}><div className="timeline-marker" aria-hidden="true" />{milestone.imageSrc ? <MediaFrame asset={{ slot: "story", src: milestone.imageSrc, alt: milestone.title, sortOrder: index, active: true, focusX: 50, focusY: 50, zoom: 1 }} className="timeline-image" alt={`Ảnh mốc ${milestone.title}`} loading="lazy" /> : null}<p className="timeline-date">{milestone.date}</p><h3>{milestone.title}</h3><p>{milestone.description}</p></li>)}</ol></div>
    </section>
