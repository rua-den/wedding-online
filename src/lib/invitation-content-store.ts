...
export const invitationContentSchema: z.ZodType<InvitationContent> = z.object({
  ...
  event: z.object({
    ...
    timeHeading: text("nhãn thời gian", 80),
    venueHeading: text("nhãn địa điểm", 80),
    directionsLabel: text("nhãn chỉ đường", 100),
  }),
  ...
  rsvp: z.object({
    eyebrow: text("nhãn RSVP", 100),
    title: text("tiêu đề RSVP", 180),
    intro: text("lời nhắc RSVP", 220),
    greetingPrefix: text("lời chào khách", 80),
    attendanceQuestion: text("câu hỏi tham dự", 180),
    attendingLabel: text("lựa chọn tham dự", 120),
    declinedLabel: text("lựa chọn không tham dự", 160),
    guestCountLabel: text("nhãn số khách", 120),
    guestCountSuffix: text("hậu tố số khách", 40),
    messageLabel: text("nhãn lời nhắn", 100),
    messagePlaceholder: text("gợi ý lời nhắn", 180),
    submitLabel: text("nút gửi RSVP", 100),
    submittingLabel: text("trạng thái đang gửi RSVP", 100),
    closedMessage: text("thông báo hết hạn RSVP", 180),
    successMessage: text("thông báo RSVP thành công", 180),
  }),
